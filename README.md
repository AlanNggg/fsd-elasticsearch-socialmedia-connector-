...
- [Network drives connector package](#network-drives-connector-package)
  ...
  - [Troubleshooting](#troubleshooting)
    - [Troubleshoot extraction](#troubleshoot-extraction)
      - [Issues extracting content from attachments](#issues-extracting-content-from-attachments)
      - [Issues extracting content from images](#issues-extracting-content-from-images)
  ...
  - [Customize extraction and syncing](#customize-extraction-and-syncing)
    - [Use document-level permissions (DLP)](#use-document-level-permissions-dlp)
      - [Enable document-level permissions](#enable-document-level-permissions)
      - [Map user identities](#map-user-identities)
      - [Sync document-level permissions data](#sync-document-level-permissions-data)
  - [Connector reference](#connector-reference)
    ...
    - [Command line interface (CLI)](#command-line-interface-cli)
      ...
      - [`deletion-sync` command](#deletion-sync-command)
      - [`permission-sync` command](#permission-sync-command)
    - [Configuration settings](#configuration-settings)
      - [`network_drive.domain` (required)](#network_drivedomain-required)
      - [`network_drive.username` (required)](#network_driveusername-required)
      - [`network_drive.password` (required)](#network_drivepassword-required)
      - [`network_drive.path` (required)](#network_drivepath-required)
      - [`network_drive.server_name` (required)](#network_driveserver_name-required)
      - [`network_drive.server_ip` (required)](#network_driveserver_ip-required)
      - [`client_machine.name` (required)](#client_machinename-required)
      - [`enterprise_search.api_key` (required)](#enterprise_searchapi_key-required)
      - [`enterprise_search.source_id` (required)](#enterprise_searchsource_id-required)
      - [`enterprise_search.host_url` (required)](#enterprise_searchhost_url-required)
      - [`enable_document_permission`](#enable_document_permission)
      - [`include/exclude`](#includeexclude)
      - [`start_time`](#start_time)
      - [`end_time`](#end_time)
      - [`log_level`](#log_level)
      - [`retry_count`](#retry_count)
      - [`network_drives_sync_thread_count`](#network_drives_sync_thread_count)
      - [`enterprise_search_sync_thread_count`](#enterprise_search_sync_thread_count)
      - [`network_drives_enterprise_search_user_mapping`](#network_drives_enterprise_search_user_mapping)
      - [Enterprise Search compatibility](#enterprise-search-compatibility)
      - [Runtime dependencies](#runtime-dependencies)
  - [Connector Limitations](#connector-limitations)
  - [if you seee Error while fetching videos. Error: \[WinError 10060\] A connection attempt failed because the connected party did not properly respond after a period of time, or established connection failed because connected host has failed to respond](#if-you-seee-error-while-fetching-videos-error-winerror-10060-a-connection-attempt-failed-because-the-connected-party-did-not-properly-respond-after-a-period-of-time-or-established-connection-failed-because-connected-host-has-failed-to-respond)
...
Record the ID of the new content source. This value is labeled _Source Identifier_ within Kibana. Later, you will [configure the connector](#configure-the-connector) with this value.
...
Begin syncing with an _incremental sync_. This operation begins [extracting and syncing content](#data-extraction-and-syncing) from network drives to Elastic. If desired, [customize extraction and syncing](#customize-extraction-and-syncing) for your use case.
...
Syncs to Enterprise Search all files _created or modified_ since the previous incremental sync.
...
Syncs to Enterprise Search all files _created or modified_ since the configured [`start_time`](#start_time). Continues until the current time or the configured [`end_time`](#end_time).

Perform this operation with the [`full-sync` command](#full-sync-command).

#### Deletion sync

Deletes from Enterprise Search all files _deleted_ since the previous deletion sync.
...
A UTC timestamp the connector uses to determine which objects to extract and sync from network drives. Determines the _starting_ point for a [full sync](#full-sync).
...
A UTC timestamp the connector uses to determine which objects to extract and sync from network drives. Determines the _stopping_ point for a [full sync](#full-sync).
...
## if you seee Error while fetching videos. Error: [WinError 10060] A connection attempt failed because the connected party did not properly respond after a period of time, or established connection failed because connected host has failed to respond

set http_proxy=http://proxy3.hkfsd.hksarg:8080
set https_proxy=http://proxy3.hkfsd.hksarg:8080
